...
public class MergeEngine {
	...
	public void apply(List localList, List remoteList, List resultList,
		...
		Map<Class<?>, List<?>> localSubTypesMap = buildSubTypesListMap(
				localList, genericClass);
		Map<Class<?>, List<?>> remoteSubTypesMap = buildSubTypesListMap(
				remoteList, genericClass);
		...
		for (Entry<Class<?>, List<?>> entry : localSubTypesMap.entrySet()) {
			this.apply(entry.getValue(), remoteSubTypesMap.get(entry.getKey()),
					resultList, entry.getKey());
		}
	...
	private <T> Map<Class<?>, List<?>> buildSubTypesListMap(
			List<T> originalList, Class<?> genericClass) {
	...
	public void setDefaultObjectMergePolicy(
			MergePolicy<?> defaultObjectMergePolicy) {

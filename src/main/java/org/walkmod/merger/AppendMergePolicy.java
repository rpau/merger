...
public class AppendMergePolicy<T extends IdentificableNode> extends ObjectMergePolicy<T> implements
    MergeEngineAware {
  ...
  public void apply(List<T> localList, List<T> remoteList,
    ...
    if (remoteList != null) {
      ...
      for (T remoteObject : remoteList) {
        ...
        @SuppressWarnings("unchecked")
        T localObject = (T) CollectionUtil.findObject(localList, remoteObject, comparator);
        if (localObject != null) {
          this.apply((T) localObject, remoteObject, updatedLocalObjects);
        } else {
          this.apply((T) localObject, remoteObject, addedRemoteObjects);
        }
      }
      if (localList != null) {
